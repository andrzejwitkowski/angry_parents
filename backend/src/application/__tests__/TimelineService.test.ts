...
describe("TimelineService", () => {
    ...
    describe("createItem", () => {
        it("should create a valid medical visit item", async () => {
            ...
            expect(item.doctor).toBe("Dr. Smith");
        });
        ...
        it("should create a medication item", async () => {
            ...
            expect(item.administered).toBe(false);
        });
    ...
    describe("getItemsByDate", () => {
        it("should return items sorted by creation time (newest first)", async () => {
            const dto1: CreateTimelineItemDto = {
                type: "NOTE",
                date: "2026-01-27",
                createdBy: "user-123",
                content: "First note",
            ...
            const dto2: CreateTimelineItemDto = {
                type: "NOTE",
                date: "2026-01-27",
                createdBy: "user-123",
                content: "Second note",
            ...
            expect(items[0].content).toBe("Second note"); // Newest first
            expect(items[1].content).toBe("First note");
        });
    ...
    describe("updateItem", () => {
        it("should update medication administered status", async () => {
            ...
            const updated = await service.updateItem(created.id, { administered: true });
        ...
        it("should throw error when updating non-existent item", async () => {
            await expect(
                service.updateItem("non-existent-id", { administered: true })
    ...
    describe("deleteItem", () => {
        ...
        it("should throw error when deleting non-existent item", async () => {
            await expect(service.deleteItem("non-existent-id")).rejects.toThrow(
