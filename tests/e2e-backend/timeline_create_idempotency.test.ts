...
import { acquireE2ETestMutex, ensureTestBackend, releaseE2ETestMutex } from "./utils/ensure";
...
describe.skipIf(!process.env.E2E_TEST)("timeline create idempotency", () => {
    ...
    beforeEach(async () => {
        ...
        const resetRes = await api.delete("/api/test/database");
        if (resetRes.status !== 200) {
            console.log("[timeline_create_idempotency] reset failed", resetRes.status, await resetRes.text());
        }

        const seedRes = await api.post("/api/test/dev/seed-mock-family", {});
        if (seedRes.status !== 200) {
            console.log("[timeline_create_idempotency] seed failed", seedRes.status, await seedRes.text());
        }

        const loginRes = await api.post("/api/auth/mock-login", { userId: "mock-user-id-dev-test-stable" });
        if (loginRes.status !== 200) {
            console.log("[timeline_create_idempotency] login failed", loginRes.status, await loginRes.text());
        }
    }, 15000);
    ...
    test("replaying POST /api/timeline with the same idempotencyKey returns the same item", async () => {
        ...
        if (childRes.status !== 200) {
            console.log("[timeline_create_idempotency] child create failed", childRes.status, await childRes.text());
        }
        ...
        if (firstRes.status !== 200) {
            console.log("[timeline_create_idempotency] first timeline create failed", firstRes.status, await firstRes.text());
        }
    ...
    test("committed create survives delayed outbox dispatch and later schedules work", async () => {
        ...
        try {
            ...
            if (childRes.status !== 200) {
                console.log("[timeline_create_idempotency] outbox child create failed", childRes.status, await childRes.text());
            }
            expect(childRes.status).toBe(200);
            const child = await childRes.json();
            ...
            if (createRes.status !== 200) {
                console.log("[timeline_create_idempotency] outbox timeline create failed", createRes.status, await createRes.text());
            }
